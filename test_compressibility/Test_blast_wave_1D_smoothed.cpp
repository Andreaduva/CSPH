...
// Here we actually simulate a blast wave resulting from a high jump in pressure alone.
// Pressure and energy values are smoothed according to a Fermi function.
// =============================================================================
...
// smoothing function:
struct Smooth {
    Smooth(double distance, double left, double right, double x_in) : d(distance), A_l(left), A_r(right), x0(x_in){};

    double operator()(double x) const {
        double num = A_l - A_r;
        double den = 1 + std::exp(x / d);
        return num / den + A_r;
    }
...
int main(int argc, char* argv[]) {
    ...
    std::string out_dir = GetChronoOutputPath() + "blast_wave_1D";
    ...
    double mass = rho_l * delta_x;

    int num_layer_l = 1;
    int num_layer_r = 1;

    double kern_multip = 1.5;  // between 1.1 and 1.4 for cubic spline kernel
    double h0 = kern_multip * delta_x;
    sysSPH.SetInitialSpacing(delta_x);
    sysSPH.SetKernelMultiplier(kern_multip);

    // ------------------------------------------------------
    // smoothing functors
    Smooth p_smooth = Smooth(delta_x / 2, p_l, p_r, 0);
    std::cout << "Pressures for x < 0:" << std::endl;
    // create fluid particles:
    std::vector<Real3> pos_l;
    for (double x = x_lim_l; x < x_lim_r; x += delta_x) {  // along x
        if (x < 0) {
            sysSPH.AddSPHParticle(ChVector3d(x, 0, 0), rho_l, p_smooth(x), p_smooth(x) / (rho_l * (gamma - 1)),
                                  ChVector3d(0), h0, mass);
            std::cout << p_smooth(x) << ", ";
        } else
            sysSPH.AddSPHParticle(ChVector3d(x, 0, 0), rho_r, p_r, e_r, ChVector3d(0), h0, mass);
        pos_l.push_back(make_Real3(x, 0, 0));
    }
    std::cout << std::endl;
    std::cout << "In the domain we have " << pos_l.size() << " particles along x" << std::endl;
    // ------------------------------------------------------------------
    ...
    double x_start_bce_l = pos_l[0].x - delta_x;
    ...
    sysFSI_csph.AddFsiBoundary(pos_bce_l, ChFrame<>(ChVector3d(0, 0, 0), QUNIT), h0, mass);
    ...
    double x_start_bce_r = pos_l.back().x + delta_x;
    for (int i = 0; i < 10; i++) {  // 10 particles on x
        double x = x_start_bce_r + i * delta_x;
        pos_bce_r.push_back(ChVector3d(x, 0, 0));
    }
    // add them as fixed markers:
    sysFSI_csph.AddFsiBoundary(pos_bce_r, ChFrame<>(ChVector3d(0, 0, 0), QUNIT), h0, mass);
    ...
    std::cout << "Particle mass is " << mass << std::endl;
    ...
    sysSPH.SetADKECoeff(0.4, 0.5, kern_multip);
    ...
    sysSPH.SetMarkerMass(rho_l * delta_x);
...
// visualization only in Release mode
    ...
    if (render) {
        // FSI plugin
        auto col_callback =
            chrono_types::make_shared<csph::ParticlePressureColorCallback_csph>(p_r - 1, p_r + 10, false);
