...
// Here initial conditions are smoothed using a Fermi function
// =============================================================================
...
int main(int argc, char* argv[]) {
    ...
    std::string out_dir = GetChronoOutputPath() + "shock_tube_1D_smoothed";
    ...
    // Initialize smoothing functors
    double delta_x = 1.5 * (d_l + d_r) / 2;
    Smooth rho_smooth = Smooth(delta_x, rho_l, rho_r, 0);
    Smooth p_smooth = Smooth(delta_x, p_l, p_r, 0);
    Smooth e_smooth = Smooth(delta_x, e_l, e_r, 0);
    // ------------------------------------------------------
    // create fluid particles on left of domain:
    std::vector<Real3> pos_l;
    for (double x = -d_l / 2; x > x_lim_l; x -= d_l) {  // along x
        //sysSPH.AddSPHParticle(ChVector3d(x, 0, 0), rho_smooth(x), p_smooth(x), e_smooth(x), ChVector3d(0), h0, rho_smooth(x)*d_l);
        sysSPH.AddSPHParticle(ChVector3d(x, 0, 0), rho_l, p_l, e_l, ChVector3d(0), h0,
                              rho_l * d_l);
    ...
    std::cout << "Particle mass on left domain is " << mass_l << " and on right side is " << mass_r << std::endl;
    ...
    std::cout << "Distance first - last = "
              << csph::Distance_csph(make_Real3(posH[0]), make_Real3(posH.back()), *manager->paramsH) << std::endl;
