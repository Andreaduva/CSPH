...
int main(int argc, char* argv[]) {
    ...
    for (double x = x_lim_l; x < x_lim_r; x += delta_x) {  // along x
        if (x < 0)
            sysSPH.AddSPHParticle(ChVector3d(x, 0, 0), rho_l, p_l, e_l, ChVector3d(0), h0, mass);
        else
    ...
    sysSPH.SetADKECoeff(1, 0.5, kern_multip);
...
// visualization only in Release mode
    ...
    if (render) {
        // FSI plugin
        auto col_callback = chrono_types::make_shared<csph::ParticlePressureColorCallback_csph>(p_r - 1, p_r + 10, false);
