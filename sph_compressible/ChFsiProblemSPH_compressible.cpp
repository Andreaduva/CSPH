...
// Modified to include compressible SPH.
// =============================================================================
...
// ----------------------------------------------------------------------------
// Constructors for base and derived classes. Builds the problem with empty SPH system.
// Consider carefully the use of m_sysSPH.
// It is declared as first attribute but here not present in the input nor in the initialization list.
// So compiler will initialize it first (because also in the list it follows the declaration order) as empty.
// So in m_sysFSI(ChFsiSystemSPH(sys, m_sysSPH)) we initialize the m_sysFSI with the
// result of ChFsiSystemSPH(sys, m_sysSPH) which uses the initialized but empty m_sysSPH variable.
// ----------------------------------------------------------------------------
ChFsiProblemSPH_csph::ChFsiProblemSPH_csph(ChSystem& sys, double spacing)
    : m_sysFSI(ChFsiSystemSPH_csph(sys, m_sysSPH)),   // empty m_sysSPH
      m_spacing(spacing),
      m_initialized(false),
      m_offset_sph(VNULL),
      m_offset_bce(VNULL),
...
ChFsiProblemCartesian_csph::ChFsiProblemCartesian_csph(ChSystem& sys, double spacing) : ChFsiProblemSPH_csph(sys, spacing) {}

ChFsiProblemCylindrical_csph::ChFsiProblemCylindrical_csph(ChSystem& sys, double spacing) : ChFsiProblemSPH_csph(sys, spacing) {}

//---------------------------------------------------
// helper functions to set parameters or quantities
//---------------------------------------------------

void ChFsiProblemSPH_csph::SetVerbose(bool verbose) {
    m_sysFSI.SetVerbose(verbose);
    m_verbose = verbose;
}

void ChFsiProblemSPH_csph::SetFluidProperties(const ChFsiFluidSystemSPH_csph::FluidProperties_csph& fluid_props) {
    m_sysSPH.SetFluidProperties(fluid_props);
}


void ChFsiProblemSPH_csph::SetSPHParameters(const ChFsiFluidSystemSPH_csph::SPHParameters_csph& sph_params) {
    m_sysSPH.SetSPHParameters(sph_params);
}

// ----------------------------------------------------------------------------
// Functions to add rigid bodies given ChBody and a geometry.
// Provided some functions to directly insert some trivial shapes
// ----------------------------------------------------------------------------

// This is a cascade method: call to AddFsiBody goes downwards to all other classes defined.
void ChFsiProblemSPH_csph::AddRigidBody(std::shared_ptr<ChBody> body,
    ...
    m_fsi_bodies[body] = fsi_body->index;  // adds the body index to the unordered map using the pointer as key.
}

void ChFsiProblemSPH_csph::AddRigidBodySphere(std::shared_ptr<ChBody> body,
...
void ChFsiProblemSPH_csph::AddRigidBodyBox(std::shared_ptr<ChBody> body, const ChFramed& frame, const ChVector3d& size) {
    auto geometry = chrono_types::make_shared<utils::ChBodyGeometry>();
...
void ChFsiProblemSPH_csph::AddRigidBodyCylinderX(std::shared_ptr<ChBody> body,
...
void ChFsiProblemSPH_csph::AddRigidBodyMesh(std::shared_ptr<ChBody> body,
                                            const ChVector3d& pos,
...
size_t ChFsiProblemSPH_csph::GetNumBCE(std::shared_ptr<ChBody> body) const {
    auto index = m_fsi_bodies.at(body);
    return m_sysSPH.m_bodies[index].bce_coords.size();
}

// ----------------------------------------------------------------------------
// Initialize function. Marks completion of FSI system and builds it. Do not add FSI bodies after its call.
// ----------------------------------------------------------------------------
// it calls some methods that are pure virtual in the base class implementation, like Grid2point() and Snap2grid().
// It also calls some new helper methods like ProcessBody() introduced here.
// So this function actually works only when called from a derived object that has all methods defined.

void ChFsiProblemSPH_csph::Initialize() {
    // Assume solid bodies added to the problem, and fluid particles grid coordinates
    // initialized in the m_sph map of the problem. SPH fluid system to be already created and its parameters set.
    // Actual sph fluid particles intead not created yet.
    //
    ...
    ChAABB aabb;  // axis-aligned bounding box. Has one min/low corner and one high/upper corner, both ChVector3d
    ...
    sph_points.reserve(m_sph.size());  // un-initialized vector
    for (const auto& p : m_sph) {
        ChVector3d point = Grid2Point(p);  // convert to real grid coordinates
        point += m_offset_sph;             // add the offset
        sph_points.push_back(point);       // adds the point at the end of the vector. Particle order is same as m_sph
        aabb += point;                     // operator+= includes given aabb in own aabb
    }

    m_sph_aabb = aabb;  // final value of m_sph_aabb

    // Convert boundary grid points to real coordinates and apply position offset
    // Include boundary BCE markers in AABB
    // Same as above but with bce markers.
    ...
    // Callback for setting initial particle properties, if not already selected we take the default one
    if (!m_props_cb)
        m_props_cb = chrono_types::make_shared<ParticlePropertiesCallback>();

    // Until now particles not actually created, the vectors in DataManager are empty.
    // Here we create SPH particles
    switch (m_sysSPH.GetPhysicsProblem()) {
        case PhysicsProblem_csph::EULER: {
            for (const auto& pos : sph_points) {  // loop over vector of particles grid positions
                m_props_cb->set(
                    m_sysSPH,
                    pos);  // set internal properties of callback class (constant properties or depth-based pressure)
                m_sysSPH.AddSPHParticle(pos, m_props_cb->rho0, m_props_cb->p0, m_props_cb->e0,
                                        m_props_cb->v0);  // default properties callback sets velocity to zero.
            }
            break;
        }
    }
...
//----------------------------------------------------------------------------
// Helper function not belonging to a class. Used by methods ProcessBody() and equivalents for meshes.
...
// ----------------------------------------------------------
// Prune fluid SPH particles inside a body volume.
// Fluid particles should not be inside a body.
// Notice it only erases sph particles from m_sph (unordered set GridPoints), not from any FsiDataManager vector
// ----------------------------------------------------------
void ChFsiProblemSPH_csph::ProcessBody(ChFsiFluidSystemSPH_csph::FsiSphBody& b) {
    int num_removed = 0;
    ...
    ChVector3i aabb_min(+std::numeric_limits<int>::max());  // initialize upper and lower limits of the integer body AABB
    ChVector3i aabb_max(-std::numeric_limits<int>::max());
    for (auto& p_abs : b.bce) {
        auto p_sph = p_abs - m_offset_sph;  // BCE point in ChFsiProblemSPH frame, m_offset_sph is a ChVector3d for the
                                            // offset of all sph particles.
        auto p_grd = Snap2Grid(p_sph);  // BCE point in integer grid coordinates. Snap2Grid defined in derived classes.
        aabb_min = Vmin(aabb_min, p_grd);  // Vmin and Vmax are 2 chrono functions. They return a vector whose component
                                           // are min/max of input components
        aabb_max =
            Vmax(aabb_max, p_grd);  // Updates and modifies the max and min values of the shape surronding the body.
    }

    //// TODO: possible performance optimization: also check if grid point inside SPH AABB

    // Collect all possible grid points inside the volume of a body primitive shape
    GridPoints interior;  // recall gridPoints is an unordered set of ChVector3i type and hashing function CoordHash.
    // traverse all possible grid locations (integer coordinates)
    for (int ix = aabb_min.x(); ix <= aabb_max.x(); ix++) {
        for (int iy = aabb_min.y(); iy <= aabb_max.y(); iy++) {
            for (int iz = aabb_min.z(); iz <= aabb_max.z(); iz++) {
                // Convert the specific grid position to body local frame
                ChVector3d p_sph = Grid2Point({ix, iy, iz});  // get the double type associated to grid point as int_coord*m_spacing
                ChVector3d p_abs = p_sph + m_offset_sph;
                ChVector3d p_loc = b.fsi_body->body->TransformPointParentToLocal(p_abs);
                // Check if the grid location inside a primitive shape with respect to the body. If yes collect it
                if (InsidePoint(*b.fsi_body->geometry, p_loc, m_spacing))  // envelope set = m_spacing
    ...
    // We loop over all grid points stored as interior and if a fluid sph particle matches that location, we remove it
    // from the system.
...
//-----------------------------------------------
int ChFsiProblemSPH_csph::ProcessBodyMesh(ChFsiFluidSystemSPH_csph::FsiSphBody& b,
...
// ----------------------------------------------------------------------
// Functions to save or output data.
// Recall we already have methods for this purpose in ChFsiFluidSystemSPH
//-----------------------------------------------------------------------

void ChFsiProblemSPH_csph::SetOutputLevel(OutputLevel output_level) {
    m_sysSPH.SetOutputLevel(output_level);
}

// save sph particles and solid data at current time/frame
void ChFsiProblemSPH_csph::SaveOutputData(double time, const std::string& sph_dir, const std::string& fsi_dir) {
    m_sysSPH.SaveParticleData(sph_dir);  // method in ChFsiFLuidSystemSPH that calls method in UtilsPrintSph. It stores
                                         // internally the current frame (counter) for the unique file name.
    m_sysSPH.SaveSolidData(fsi_dir, time);  // recall Solid data and sph particle data have different save formats. See UtilsPrintSph for details.
}

void ChFsiProblemSPH_csph::SaveInitialMarkers(const std::string& out_dir) const {
...
// ----------------------------------------------------------------------------
// Advance dynamics of fsi system to "time + step"
void ChFsiProblemSPH_csph::DoStepDynamics(double step) {
    m_sysFSI.DoStepDynamics(step);  // fsi system function to advance both MBS and CFD dynamics
    // actual time not actually needed. Integrating from "time" to "time+step" is like integrating from 0 to step
    // provided initial conditions are properly set
}

// ----------------------------------------------------------------------------

const ChVector3d& ChFsiProblemSPH_csph::GetFsiBodyForce(std::shared_ptr<ChBody> body) const {
    auto index = m_fsi_bodies.at(body);
    return m_sysFSI.GetFsiBodyForce(index);
}

const ChVector3d& ChFsiProblemSPH_csph::GetFsiBodyTorque(std::shared_ptr<ChBody> body) const {
    auto index = m_fsi_bodies.at(body);
    return m_sysFSI.GetFsiBodyTorque(index);
}

// ------------------------------------------
// functions related to particle relocation
//-------------------------------------------

void ChFsiProblemSPH_csph::CreateParticleRelocator() {
    FsiParticleRelocator_csph::DefaultProperties_csph props;
    props.rho0 = m_sysSPH.GetBaseDensity();
    props.e0 = m_sysSPH.GetBaseEnergy();

    m_relocator = chrono_types::make_unique<FsiParticleRelocator_csph>(*m_sysSPH.m_data_mgr, props);
}

void ChFsiProblemSPH_csph::BCEShift(const ChVector3d& shift_dist) {
    ChAssertAlways(m_relocator);

    m_relocator->Shift(MarkerType::BCE_WALL, ToReal3(shift_dist));
}

void ChFsiProblemSPH_csph::SPHShift(const ChVector3d& shift_dist) {
    ChAssertAlways(m_relocator);

    m_relocator->Shift(MarkerType::SPH_PARTICLE, ToReal3(shift_dist));
}

void ChFsiProblemSPH_csph::SPHMoveAABB2AABB(const ChAABB& aabb_src, const ChIntAABB& aabb_dest) {
    ChAssertAlways(m_relocator);

    m_relocator->MoveAABB2AABB(MarkerType::SPH_PARTICLE, ToRealAABB(aabb_src), ToIntAABB(aabb_dest), Real(m_spacing));
}

void ChFsiProblemSPH_csph::ForceProximitySearch() {
    m_sysSPH.m_force_proximity_search = true;
}


// ============================================================================
// Methods of derived class ChFsiProblemCartesian
// ============================================================================

// reads grid position of sph particles and bce markers from two separate input files

void ChFsiProblemCartesian_csph::Construct(const std::string& sph_file, const std::string& bce_file, const ChVector3d& pos) {
    if (m_verbose) {
...
// construct problem inside a box of input dimensions

void ChFsiProblemCartesian_csph::Construct(const ChVector3d& box_size, const ChVector3d& pos, int side_flags) {
...
// construct problem from input heatmap file
void ChFsiProblemCartesian_csph::Construct(const std::string& heightmap_file,
                                           double length,
                                           double width,
...
size_t ChFsiProblemCartesian_csph::AddBoxContainer(const ChVector3d& box_size,  // box dimensions
...
// -------------------------------------------------------------------------------
// Snap2grid and Grid2Point methods needed to be defined for class to be concrete.
// -------------------------------------------------------------------------------

// Given a double vector of coordinates, returns the equivalent grid position in integer coordinates
ChVector3i ChFsiProblemCartesian_csph::Snap2Grid(const ChVector3d& point) {
    return ChVector3i((int)std::round(point.x() / m_spacing),  //
                      (int)std::round(point.y() / m_spacing),  //
                      (int)std::round(point.z() / m_spacing));
}

// Given integer grid coordinates, returns the corresponding double values.
// Obtained simply by multiplying by the particle spacing
ChVector3d ChFsiProblemCartesian_csph::Grid2Point(const ChVector3i& p) {
    return ChVector3d(m_spacing * p.x(), m_spacing * p.y(), m_spacing * p.z());
}

// ============================================================================
// Define Construct() and Snap2Grid() methods for ChFsiProblemCylindrical class
// ============================================================================

void ChFsiProblemCylindrical_csph::Construct(double radius_inner,
                                        double radius_outer,
                                        double height,
                                        const ChVector3d& pos,
                                        int side_flags) {
...
size_t ChFsiProblemCylindrical_csph::AddCylindricalContainer(double radius_inner,
                                                             double radius_outer,
                                                             double height,
                                                             const ChVector3d& pos,
                                                             int side_flags) {
...
ChVector3i ChFsiProblemCylindrical_csph::Snap2Grid(const ChVector3d& point) {
...
ChVector3d ChFsiProblemCylindrical_csph::Grid2Point(const ChVector3i& p) {
    if (p.x() == 0) {
