...
// =============================================================================
// Modification of original class to include compressible SPH data structures
...
class CH_FSI_API ChFsiVisualizationVSG_csph : public vsg3d::ChVisualSystemVSGPlugin {
  public:
    // Rendering mode for particles and mesh objects.
    enum class RenderMode { POINTS, WIREFRAME, SOLID };

    ChFsiVisualizationVSG_csph(ChFsiSystemSPH_csph* sysFSI);
    ChFsiVisualizationVSG_csph(ChFsiFluidSystemSPH_csph* sysSPH);

    ~ChFsiVisualizationVSG_csph();
    ...
    class CH_FSI_API ParticleColorCallback_csph : public ChParticleCloud::ColorCallback {
      public:
        ...
        ChFsiVisualizationVSG_csph* m_vsys;
        Real3* pos;
        Real3* vel;
        Real3* prop;
    ...
    void SetSPHColorCallback(std::shared_ptr<ParticleColorCallback_csph> functor,
  ...
  private:
    ...
    ChFsiSystemSPH_csph* m_sysFSI;       //< associated FSI system
    ChFsiFluidSystemSPH_csph* m_sysSPH;  //< associated SPH system
    ...
    std::shared_ptr<ParticleColorCallback_csph> m_color_fun;         //< color functor for SPH particles
    ...
    std::vector<Real3> m_vel;   //< SPH and BCE velocities
    std::vector<Real3> m_acc;   //< SPH and BCE accelerations
    std::vector<Real3> m_frc;   //< SPH and BCE forces
    std::vector<Real3> m_prop;  //< SPH properties (density, pressure, energy)
    ...
    friend class FSIStatsVSG_csph;
};

// -----------------------------------------------------------------------------

// Predefined SPH coloring based on particle height.
class CH_FSI_API ParticleHeightColorCallback_csph : public ChFsiVisualizationVSG_csph::ParticleColorCallback_csph {
  public:
    ParticleHeightColorCallback_csph(double hmin, double hmax, const ChVector3d& up = ChVector3d(0, 0, 1));
...
class CH_FSI_API ParticleVelocityColorCallback_csph : public ChFsiVisualizationVSG_csph::ParticleColorCallback_csph {
  public:
    enum class Component { X, Y, Z, NORM };

    ParticleVelocityColorCallback_csph(double vmin, double vmax, Component component = Component::NORM);
...
class CH_FSI_API ParticleDensityColorCallback_csph : public ChFsiVisualizationVSG_csph::ParticleColorCallback_csph {
  public:
    ParticleDensityColorCallback_csph(double dmin, double dmax);
...
class CH_FSI_API ParticlePressureColorCallback_csph : public ChFsiVisualizationVSG_csph::ParticleColorCallback_csph {
  public:
    ParticlePressureColorCallback_csph(double pmin, double pmax, bool bimodal);
...
// Predefined SPH coloring based on particle temperature.
class CH_FSI_API ParticleEnergyColorCallback_csph : public ChFsiVisualizationVSG_csph::ParticleColorCallback_csph {
  public:
    ParticleEnergyColorCallback_csph(double emin, double emax, bool bimodal);
